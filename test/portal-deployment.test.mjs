...
test("Portal image no longer includes updater agents or their local transaction libraries", async () => {
  const dockerfile = await readFile(new URL("../Dockerfile", import.meta.url), "utf8");
  const dockerignore = await readFile(new URL("../.dockerignore", import.meta.url), "utf8");
  const copied = dockerfile.split("\n").filter((line) => line.startsWith("COPY ")).join("\n");
  assert.doesNotMatch(copied, /node-updater|packages\/codey\/lib/);
  assert.doesNotMatch(dockerignore, /!.*node-updater|!.*lib\/update-/);
});
