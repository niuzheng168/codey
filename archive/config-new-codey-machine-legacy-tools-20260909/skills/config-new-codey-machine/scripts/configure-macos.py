...
common = module("codey_machine_common", "machine-common.py")
tunnel = module("codey_macos_tunnel_client", "tunnel-client.py")
...
def token_bound_tunnel(executable, enrollment, config_root):
    binding = tunnel.ensure_tunnel(executable, enrollment, config_root)
...
def configure(args):
    ...
    if not args.apply:
        ...
        return
    logged_in = run([devtunnel, "user", "show", "--json"], check=False)
    if not tunnel.github_logged_in(logged_in):
    ...
    try:
        ...
        write_private(config_root / "tunnel-client.py", (SKILL / "scripts/tunnel-client.py").read_text())
        ...
        config = {
            ...
            "tunnelHelper": str(config_root / "tunnel-client.py"),
            "tunnelHelperSha256": digest(config_root / "tunnel-client.py"),
        }
