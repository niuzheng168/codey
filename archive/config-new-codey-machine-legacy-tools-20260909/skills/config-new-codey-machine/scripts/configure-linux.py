#!/usr/bin/env python3
"""Plan/install a fresh Linux x64 node using a private GitHub-authenticated DevTunnel."""
import argparse
import base64
import hashlib
import http.client
...
common = load("codey_linux_common", "machine-common.py")
tunnel = load("codey_linux_tunnel_client", "tunnel-client.py")
worker = load("codey_linux_tunnel_worker", "linux-tunnel-service.py")
SetupError = common.SetupError
run = common.run
protected_write = common.protected_write
digest = common.digest
prepare_runtime = common.prepare_runtime
...
def configure(args):
    ...
    executable = worker.prepare_cli(args.devtunnel_bin or shutil.which("devtunnel"), SKILL, enrollment["nodeId"])
    try:
        tunnel.require_github_login(executable)
    ...
    try:
        ...
        tunnel.require_github_login(executable)
        binding = tunnel.ensure_tunnel(executable, enrollment, config)
        network["devTunnel"] = binding
        protected_write(config / "enrollment.json", json.dumps(enrollment, indent=2) + "\n")
        for file in ("linux-tunnel-service.py", "tunnel-client.py"):
            protected_write(config / file, (SKILL / "scripts" / file).read_text())
        runtime = {
            ...
            "devtunnelSha256": digest(executable), "worker": str(config / "linux-tunnel-service.py"),
            "workerSha256": digest(config / "linux-tunnel-service.py"),
            "tunnelHelper": str(config / "tunnel-client.py"), "tunnelHelperSha256": digest(config / "tunnel-client.py"),
            **binding,
        }
        runtime_file = config / "tunnel-runtime.json"
        protected_write(runtime_file, json.dumps(runtime, indent=2) + "\n")
        tunnel.renew(runtime, enrollment, force=True)
        ...
        for mode, name in (("host", "codey-devtunnel.service"), ("renew", "codey-devtunnel-renew.service")):
            content = unit("Codey private GitHub tunnel " + mode,
                           f"{Path(sys.executable).resolve()} -I -B {config}/linux-tunnel-service.py {mode} {runtime_file}",
...
if __name__ == "__main__":
    try:
        configure(arguments())
    except (SetupError, worker.ServiceError, tunnel.TunnelError, FileExistsError, KeyError, ValueError) as error:
