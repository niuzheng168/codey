    ...
    with lock.open("a+b") as stream:
        ...
        while True:
            ...
            if args.component == "tunnel":
                child_env = tunnel.cli_environment(child_env)
                if config.get("tunnelAuthProvider") == "github":
                    try:
                        tunnel.require_github_login(config["devtunnelExe"])
            ...
            with subprocess.Popen(
                commands[args.component], cwd=config["workspaceRoot"], env=child_env,
