...
2. 在私有新目录解压 Codey 应用，逐文件校验压缩包与安装结果。依赖锁相同时直接
   链接已检查的现有依赖，不复制依赖文件、不访问 npm；锁改变时才执行锁定安装。
   使用原 Node 执行 `codey doctor`，不继承模型凭据、`NODE_OPTIONS` 或 npm 覆盖参数。
3. 排除其他本地更新；校验服务/配置基线，确认空闲后才切换。
4. 验证新服务，保存版本记录。失败只恢复本次代码指针/运行描述符/版本标记，
   不恢复旧数据库或旧密钥，不覆盖另一部署者的修改。

0.1.6/0.1.7 的本地 CLI 使用依赖副本。新版常规更新改为直接复用已安装的依赖树：
Windows 使用目录 junction，Linux 使用目录链接，不复制数万个依赖文件，不运行
npm install/rebuild 或依赖 hooks。链接绑定依赖图、原用户目录、平台与 Node ABI，
不能指向任意路径；只修改应用包，依赖树在升级期间不写入。
普通 npm 安装换目录时，链接会指向保留的回滚目录；后续更新直接复用该实际依赖树，
不形成层层链接。不要手动删除仍被引用的旧依赖目录，也不要在共享依赖中运行 npm 修改。
`--check` 报告 `dependencyMode`；加 `--offline` 可要求只使用此路径。
轻量 npm 包仍只携带应用文件，不携带 Node、node_modules 或离线依赖缓存。
仅在需要完全独立的新安装时，安装器 `--check --reuse-from OLD_ROOT` 仍复制依赖，
避免新安装依赖一个可被单独删除的旧安装。这不是常规更新流程。旧 CLI 可由其引导，
