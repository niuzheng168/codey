...
主页检查的是门户读取用量接口的数据链路，全部失败时显示“数据链路异常”，不据此
断言整机离线。它与节点主动上报的升级器心跳是两个独立状态。
...
移除客户端注册节点后，原账号可重新上传该机器的有效注册文件恢复接入。服务端保留原
owner、节点 ID、密钥、平台及隧道绑定；重新验通后可接受安装脚本重跑生成的新 TLS 证书，
但不会用注册重试替换仍在使用的节点证书。恢复沿用原显示设置和升级器记录，仍受节点
数量及隧道唯一性限制；验通或升级器接入未完成时不开放节点访问，其他账号不能认领。
数据与 Workspace 网关缓存同时绑定节点 ID、服务入口、TLS 证书和隧道坐标；节点移除
或绑定变化时立即淘汰旧连接及对应健康快照，不能继续使用删除前的证书。普通显示信息
修改与令牌续期保留健康连接，避免影响其他节点或中断正常 Workspace 会话。
