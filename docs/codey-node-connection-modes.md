...
## 当前源码 UI：仅私有 DevTunnel

Usage 和 Workspace 固定使用已分配给当前用户、由服务端声明的 DevTunnel 节点。
浏览器只访问同源门户 API，门户再使用私有隧道连接节点；不会因隧道失败退回
VNet、节点公网地址或浏览器回环地址。旧 localStorage 的连接偏好不再控制页面。

VNet 勾选框、本机重试按钮与手动 HTTPS 地址编辑已从默认 UI 移除。
旧节点仍可在设置中修改显示信息或移除，未配置隧道时提示重新接入，不改写原连接配置。
`public/portal-features.js` 将旧界面保持关闭；底层传输、API 和兼容测试仍保留。

下面保留 2026-09-05 的历史方案与部署验收记录，不代表当前页面仍提供相应入口。
