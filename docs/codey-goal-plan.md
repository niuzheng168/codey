...
### 会话中的 Goal 状态栏（2026-09-12）

打开已有 Codex 会话时，聊天页顶部会主动读取并固定显示原生 Goal，包括在
Codex 桌面端或其他客户端启动的目标。目标、状态、已用 Token、预算和耗时
均来自原生快照；这里的 Token 是 **Goal 自身用量**，不是整个会话的累计用量。
长目标可展开查看，不依赖聊天历史分页，也不会随消息滚出页面。

前台聊天页对 active Goal 每 5 秒刷新，其他状态/无目标时每 30 秒检查；
重新连接、返回页面或会话更新也会触发合并后的刷新。隐藏聊天页和后台标签页
不持续轮询。读取失败保留上次确认的目标并明确标记旧状态；旧节点不支持接口时
显示升级提示，不把失败误报为“没有目标”。切换会话会取消旧请求并隔离迟到响应。

状态栏只调用已有 `/goal status` 读取路径，不发送提示词，不创建、恢复、暂停
或接管目标。已有原生 goal 接口的节点无需为此状态栏更新后端；控制仍使用上述
斜杠命令。

通过网页启动的目标仍有一个持续更新的聊天状态条目。Codey 跟踪原生调度器的连续轮次，
