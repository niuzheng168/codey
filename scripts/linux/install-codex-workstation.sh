...
Options:
  ...
  --skip-start        Install and enable the service without starting it
...
for command in node npm sha256sum systemctl curl openssl base64; do
...
if [[ -z "$PACKAGE_PATH" ]]; then
  ...
  printf '%s' "$EMBEDDED_PACKAGE_BASE64" | base64 --decode > "$PACKAGE_PATH"
fi
...
if ! systemctl --user show-environment >/dev/null 2>&1; then
  printf 'A working systemd user manager is required.\n' >&2
  exit 1
fi
...
export PATH="$runtime_bin:/usr/local/bin:/usr/bin:/bin:${PATH:-}"

copilot_home="$HOME/.local/share/copilot-api"
codex_home="$HOME/.codex"
service_dir="$HOME/.config/systemd/user"
service_path="$service_dir/copilot-api.service"
...
existing=()
for candidate in "$config_path" "$codex_config_path" "$models_path" "$service_path" "$package_json"; do
  [[ -e "$candidate" ]] && existing+=("$candidate")
done
if ((${#existing[@]} > 0 && FORCE == 0)); then
  printf 'Existing installation detected. Re-run with --force to back up configuration:\n' >&2
  printf '  %s\n' "${existing[@]}" >&2
  exit 1
fi

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
for candidate in "$config_path" "$codex_config_path" "$models_path" "$service_path" "$key_path"; do
  if [[ -f "$candidate" ]]; then
...
npm install --global "$install_package"
...
mkdir -p "$copilot_home" "$codex_home" "$service_dir"
chmod 700 "$copilot_home" "$codex_home" "$service_dir"
...
profile_marker="# Codex workstation managed PATH"
if ! grep -Fq "$profile_marker" "$profile_path" 2>/dev/null; then
  {
    printf '\n%s\n' "$profile_marker"
...
[Unit]
Description=Copilot API for Codex
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
Environment=PATH=$runtime_bin:$node_dir:/usr/local/bin:/usr/bin:/bin
ExecStart=$copilot_bin start --port $PORT
Restart=always
RestartSec=3
...
if ((SKIP_START == 1)); then
  systemctl --user enable copilot-api.service
else
  printf '==> Starting Copilot API\n'
  ...
  if ((ready == 0)); then
    journalctl --user -u copilot-api.service -n 40 --no-pager >&2 || true
