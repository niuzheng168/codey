...
原 npm 的 pacote 暂存应用；依赖锁相同时直接引用已检查、保留的现有依赖，不整树复制或重装、不访问 npm 源。
锁不同时使用 `npm ci`，依赖与原生预编译文件可能联网下载，但不会安装缺少的
编译工具。切换后验证目标版本、启动状态和本机鉴权健康。
**不自动发送模型请求、创建合成会话或运行 Codex CLI 推理，同包校验也不例外。**
`authenticated-health-v1` 请求生成绑定任务/签名摘要/版本/入口哈希的健康证明，
明确记录 `modelRequests: false`，不伪造模型成功。失败只回退本次代码/描述符，不恢复旧数据库、
密钥或覆盖并发部署。Portal 只在验证完成后显示成功。

## 中断、撤销与恢复

重启/代理崩溃后先恢复本机未完成事务，再依赖 Portal；不会重新发送已验证的模型调用。
历史完成请求没有新验收标记时，仍须原有真实模型证明；旧未完成事务安全回退，
不重新推理、不将已失败/回退任务改成成功。保留被依赖引用的旧包与备份。
...
确认原生模块、已鉴权健康、代理重启与回滚路径后，再进行批量升级；不自动进行模型测试。
