...
def inventory(source):
    ...
    files = {}
    for name in FILES:
        file = ordinary_path(source.parent / name)
        if file.exists():
            if not file.is_file():
...
def verify(snapshot, root, node_id):
    ...
    if (not isinstance(files, dict) or "codex.exe" not in files or set(files) - set(FILES)
    ...
    if snapshot.get("executable") != str(directory / "codex.exe"):
    ...
    if (not saved.is_file() or saved.stat().st_size > 16384
            ...
            or {file.name for file in directory.iterdir()} != set(files) | {"bundle.json"}):
...
def pin(source, root, node_id, *, apply=False):
    ...
    snapshot = {
        ...
        "sourceExecutable": str(source), "executable": str(directory / "codex.exe"), "files": files,
    }
    if directory.exists():
    ...
    for name, expected in files.items():
        original = ordinary_path(source.parent / name)
