"""Stop and archive the current owner's existing Codey installation."""
import json
import os
...
PREVIOUS_PATHS = (
    ("previous-cloudcli-runtime", ".local/share/codey-cloudcli"),
    ("previous-cloudcli-config", ".config/codey-cloudcli"),
    ("previous-copilot-api-runtime", ".local/share/copilot-api"),
    ("previous-updater-runtime", ".local/share/codey-updater"),
    ("previous-updater-config", ".config/codey-updater"),
    ("previous-relay-runtime", ".local/share/codey-node-relay"),
    ("previous-relay-config", ".config/codey-node-relay"),
    ("previous-codex-tools", ".local/share/codey-tools/codex"),
)
...
def inspect(home, root, config, runner, *, port_available=_port_available):
    """Return a read-only replacement plan; unknown listeners still fail closed."""
    ...
    if occupied and not any(unit["active"] and unit["pid"] > 0 for unit in units):
        raise SetupError("Required ports are occupied by an unknown process; the installer will not stop it")
    paths = []
    candidates = (("runtime", root), ("config", config),
                  *((label, home / relative) for label, relative in PREVIOUS_PATHS))
...
def execute(home, root, config, node_id, runner, *, port_available=_port_available):
    ...
    if any(path.lstat().st_dev != device for path in sources):
        raise SetupError("Existing paths cannot be atomically archived on this filesystem")
    ...
    for item in plan["units"]:
        ...
        if result.returncode:
            raise SetupError(f"Could not stop the existing unit {item['name']}")
    runner(["systemctl", "--user", "daemon-reload"])
    for item in plan["units"]:
        record = _show(runner, item["name"])
        if record.get("ActiveState") == "active" or record.get("MainPID", "0") not in ("", "0"):
            raise SetupError(f"Existing unit did not stop: {item['name']}")
    ...
    if remaining:
        raise SetupError("A listener remains after stopping existing Codey services: "
    ...
    runner(["systemctl", "--user", "daemon-reload"])
    protected_write(backup / "replacement.json", json.dumps({
