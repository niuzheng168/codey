...
def ensure_tunnel(executable, enrollment, config_root, *, runner=subprocess.run):
    ...
    if journal.exists():
        ...
        qualified = state.get("qualifiedId", requested)
        if qualified != requested and (not coordinates(state)
                or qualified != state["tunnelId"] + "." + state["clusterId"]):
    ...
    if not coordinates(tunnel) or tunnel.get("description") != description:
        raise TunnelError("tunnel_not_bound_to_this_installation")
    qualified = tunnel["tunnelId"] + "." + tunnel["clusterId"]
    write_state(journal, {"requested": requested, "qualifiedId": qualified,
                          "tunnelId": tunnel["tunnelId"], "clusterId": tunnel["clusterId"]})
    ports = tunnel.get("ports") or []
    if any(port.get("portNumber") not in (3001, 8443) for port in ports):
    ...
    return {"tunnelId": tunnel["tunnelId"], "clusterId": tunnel["clusterId"]}
...
def host_connections(config, *, runner=subprocess.run):
    ...
    try:
        ...
        if value["tunnelId"] != config["tunnelId"] or type(count) is not int or not 0 <= count <= 64:
            return None
        return count
